...
namespace zketch {
	...
	std::optional<Font> TryCreateFont(HDC hdc, const std::wstring& fontname, int reqWeight, bool reqItalic) noexcept ;
	...
	#pragma pack(pop)
		...
		[[nodiscard]] inline std::optional<std::pair<size_t, size_t>> DumpInstalledFonts(const DumpOptions& opt) noexcept {
			...
			if (opt.write_csv) {
				...
				if (opt.write_header) {
					csvOutput.append("Font Name,Height,Ascent,Descent,Weight,Style, 32, 33, 34, chars...\n") ;
				}
			}
