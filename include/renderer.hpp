...
namespace zketch {
	...
	public :
		...
		void Present(const Window& window) const noexcept {
			if (!window.front_buffer_ || !window.front_buffer_->IsValid()) {
				logger::warning("Window::Display - invalid canvas") ;
				return ;
			}

			HDC hdc = GetDC(window.handle_) ;
			if (!hdc) {
			...
			screen.DrawImage(window.front_buffer_->GetBitmap(), 0, 0) ;

			ReleaseDC(window.handle_, hdc) ;
		}
