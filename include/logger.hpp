    ...
    private :
        ...
        static inline void set_color(int32_t lv) noexcept {
            ...
            switch (lv) {
                ...
                default : attr = 7; break ; // Default color
            }
        ...
        template <typename T>
        static inline void append_narrow(std::string& out, T&& v) { // Terima dengan forwarding reference
            ...
            if constexpr (std::is_same_v<U, std::string> || std::is_same_v<U, std::string_view> || std::is_same_v<U, const char*>) {
            ...
            } else if constexpr (std::is_enum_v<U>) { // <-- PERBAIKAN UNTUK ENUM
                append_narrow(out, static_cast<std::underlying_type_t<U>>(v)) ;
                ...
                static_assert(sizeof(T) != sizeof(T), "Unsupported type for append_narrow; convert to string first") ; // Sedikit diubah biar lebih jelas
            }
        }
        ...
        static inline void widen_utf8_to_wide(const char* src, int src_len, std::wstring& dst) {
            ...
            dst.resize(old_size + needed) ;
            MultiByteToWideChar(CP_UTF8, 0, src, src_len, dst.data() + old_size, needed) ;
        }
        ...
        template <typename T>
        static inline void append_wide(std::wstring& out, T&& v) { // Terima dengan forwarding reference
            using U = std::decay_t<T> ;
            ...
            if constexpr (std::is_same_v<U, std::wstring> || std::is_same_v<U, std::wstring_view> || std::is_same_v<U, const wchar_t*>) {
                ...
                if constexpr (std::is_integral_v<U>) {
                    auto [p, ec] = std::to_chars(buf, buf + sizeof(buf), v) ;
                    if(ec == std::errc()) widen_utf8_to_wide(buf, static_cast<int>(p - buf), out) ;
                } else {
                    auto [p, ec] = std::to_chars(buf, buf + sizeof(buf), v, std::chars_format::general) ; 
                    if(ec == std::errc()) widen_utf8_to_wide(buf, static_cast<int>(p - buf), out) ;
                }
            } else if constexpr (std::is_same_v<U, std::string> || std::is_same_v<U, std::string_view> || std::is_same_v<U, const char*>) {
                widen_utf8_to_wide(std::string_view{v}, out) ;
            } else if constexpr (std::is_enum_v<U>) { // <-- PERBAIKAN UNTUK ENUM
                append_wide(out, static_cast<std::underlying_type_t<U>>(v));
            } else if constexpr (std::is_pointer_v<U>) {
                uintptr_t addr = reinterpret_cast<uintptr_t>(v) ;
                char buf[2 + (sizeof(uintptr_t) * 2) + 1] ;
                buf[0] = '0'; buf[1] = 'x' ;
                ...
                static_assert(sizeof(T) != sizeof(T), "Unsupported type for append_wide; convert to wstring first") ;
            }
        }
        ...
        template <typename ... Args>
        static inline void log_wide(int32_t lv, const wchar_t* tag, Args&& ... args) noexcept { // <-- PERBAIKAN: forwarding reference
            std::wstring buf ;
            buf.reserve(256) ;
            buf.push_back(L'[') ;
            buf.append(tag) ;
            buf.append(L"]\t") ;
            (append_wide(buf, std::forward<Args>(args)), ...) ; // <-- PERBAIKAN: std::forward
            buf.push_back(L'\n') ;
